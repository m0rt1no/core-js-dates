...
function getTime(date) {
  const time = { hourCycle: 'h23', timeStyle: 'medium' };
  return new Intl.DateTimeFormat('en-US', time).format(new Date(date));
}
...
function getDayName(date) {
  const d = { weekday: 'long' };
  return new Intl.DateTimeFormat('en-US', d).format(new Date(date));
}
...
function getNextFriday(date) {
  ...
  const untilFriday = (5 - dayOfWeek + 7) % 7 || 7;
  nextFriday.setDate(date.getDate() + untilFriday);
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return check >= firstDay && check <= lastDay;
}
...
function formatDate(date) {
  ...
  return new Intl.DateTimeFormat('en-US', obj).format(new Date(date));
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((target - firstDay) / (24 * 60 * 60 * 1000) + 1) / 7);
}
